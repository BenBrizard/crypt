...
{
	//hex index donne l'index dans "0123456789abcdef"
...
bool Hex::decodeHex(unsigned char *input, unsigned char *output, uint32_t length)
{
	if (!Hex::validateHex(input, length))
		return INVALID_HEX;
	for (int i = 0; i < length; i += 2)// <length, car on fait bond de 2
	{
